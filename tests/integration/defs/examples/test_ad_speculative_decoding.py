...
def test_nemotron_mtp_model_with_weights():
    ...
    if mtp_model_path is None:
        pytest.skip(f"Model {mtp_model_name} not found under LLM_MODELS_ROOT")
    mtp_path = Path(mtp_model_path)
    if not mtp_path.exists():
        pytest.skip(f"Configured model path does not exist: {mtp_model_path}")
    ...
    if index is None:
        pytest.skip(
            "Safetensors index is missing/invalid or a Git-LFS pointer at configured model path: "
            f"{index_path}"
        )
