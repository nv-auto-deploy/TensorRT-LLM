...
# Test prompts (from run_llama_spec_dec.py)
prompts = [
...
# /lustre/fs1/portfolios/coreai/projects/coreai_comparch_autodeploy/autodeploy_data/hf_home/
...
def test_autodeploy_spec_dec(batch_size):
    ...
    for i, ((spec_prompt, spec_output), (baseline_prompt, baseline_output)) in enumerate(
        zip(spec_outputs, baseline_outputs)
    ):
        print(f"\n[Output {i}]")
