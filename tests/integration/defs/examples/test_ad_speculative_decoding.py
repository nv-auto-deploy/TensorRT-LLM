...
class PrefillOnlyEagleResourceManager:
    ...
    This is a temporary placeholder that allocates hidden state buffers but does not
    implement the full hidden state capture/management logic. Use this when testing
    the one-model flow before ADHiddenStateManager is ready for hidden state capture.
    """
    ...
    def __init__(
        ...
        self.num_capture_layers = num_capture_layers
...
class LlamaModelWithCapture(LlamaModel):
    ...
    Unlike the default output_hidden_states which replaces the last layer's
    un-normalized output with the normalized version, this class captures
    the true un-normalized outputs at each specified layer.
    """
...
class LlamaForCausalLMWithCapture(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        outputs = self.model(
            input_ids=input_ids, input_embeds=input_embeds, position_ids=position_ids, **kwargs
        )
...
def create_llama_model_with_capture(
    model_name: str, resource_manager, capture_layers, dtype=torch.bfloat16
) -> LlamaForCausalLMWithCapture:
    ...
    # Wrap with lm_head for logits output
    return LlamaForCausalLMWithCapture(base_model, capture_model)
...
def build_eagle_wrapper(
    ...
    target_model = create_llama_model_with_capture(
    ...
    print(f"  Config type: {type(draft_model.config).__name__}")
    ...
    print(f"  Draft model dtype: {draft_model.dtype}")
    # draft_model.to(draft_model.dtype)
    draft_model.eval()
    print("✓ Created draft model with weights loaded via factory")
    ...
    # Build EagleWrapper (this also loads weights from target into draft model)
...
def verify_eagle_wrapper_output(output, tokenizer, batch_size, num_previously_accepted):
    ...
    assert hasattr(output, "num_accepted_tokens"), "Output should have num_accepted_tokens"
    assert hasattr(output, "num_newly_accepted_tokens"), (
        "Output should have num_newly_accepted_tokens"
    )
    ...
    print(f"  num_accepted_tokens: {output.num_accepted_tokens}")
    print(f"  num_newly_accepted_tokens: {output.num_newly_accepted_tokens}")

    # Verify num_accepted_tokens is within expected bounds
    assert output.num_accepted_tokens.shape == (batch_size,), (
        f"num_accepted_tokens shape should be ({batch_size},), "
        f"got {output.num_accepted_tokens.shape}"
    )
    assert (output.num_newly_accepted_tokens >= torch.zeros_like(num_previously_accepted)).all(), (
        "num_newly_accepted_tokens should be >= 0"
    )

    # Verify: num_previously_accepted + num_newly_accepted_tokens == num_accepted_tokens
    expected_accepted = num_previously_accepted + output.num_newly_accepted_tokens
    assert torch.equal(expected_accepted, output.num_accepted_tokens), (
        f"num_previously_accepted + num_newly_accepted_tokens != num_accepted_tokens\n"
        f"  num_previously_accepted: {num_previously_accepted}\n"
        f"  num_newly_accepted_tokens: {output.num_newly_accepted_tokens}\n"
        f"  expected: {expected_accepted}\n"
        f"  actual num_accepted_tokens: {output.num_accepted_tokens}"
    )
...
def test_eagle_wrapper_forward(batch_size: int):
    ...
    if batch_size == 1:
        input_ids = tokenizer.encode(test_prompts[0], return_tensors="pt").to(device)
    else:
        tokenized = tokenizer(
    ...
    print(f"  test_prompts: {test_prompts[:batch_size]}")
    print(f"  input_ids: {input_ids}")
    ...
    resource_manager = PrefillOnlyEagleResourceManager(
        ...
        max_draft_len=max_draft_len,
        target_dtype=dtype,
        draft_dtype=torch.float16,  # hardcoded from eagle3 model
    )
    ...
    print("✓ num_accepted_tokens within expected bounds")
    ...
    # Dictionary to track distribution of num_newly_accepted_tokens
    ...
    for iteration in range(num_iterations):
        ...
        valid_prefix_len = output.num_accepted_tokens + max_draft_len
        next_target_inputs = [
            torch.cat(
        ...
        # Track distribution of newly accepted tokens by reading num_newly_accepted_tokens from the output.
        # For batch size = 1, we are inputting draft tokens to the wrapper model, so num_newly_accepted_tokens
        # gives the number of accepted tokens from drafts in the previous iteration.
        if batch_size == 1:
            for val in output.num_newly_accepted_tokens.tolist():
        ...
        if batch_size > 1:
            ...
            num_previously_accepted = output.num_accepted_tokens + 1
    ...
    # For batch_size == 1: uses num_newly_accepted_tokens from eagle wrapper
    ...
    # o this provides a check that they are "similar enough" to each other.
