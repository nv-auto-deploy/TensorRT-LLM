        ...
        "targets": [
            {
                ...
                "source": {
                    "yaml": "tests/integration/test_lists/test-db/l0_h100.yml",
                },
                "tags": ["backend:autodeploy"],
                "target_id": "//ci_target_graph/l0_h100:synthetic_triton_case",
            }
        ],
    }
...
def test_generate_autodeploy_h100_bazel_cases_from_manifest() -> None:
    ...
    assert AUTODEPLOY_RUNTIME_DEP in h100_case.deps
    assert ACCURACY_TESTS_DEP in h100_case.deps
    _assert_native_artifact_deps(h100_case.deps)
    ...
    assert AUTODEPLOY_RUNTIME_DEP in multi_path_case.deps
    _assert_native_artifact_deps(multi_path_case.deps)


def test_generate_autodeploy_h100_bazel_cases_include_native_deps_in_stable_order() -> None:
    cases = bazel_cases_from_manifest(_build_manifest())
...
def test_generate_autodeploy_h100_bazel_case_with_triton_runtime_depends_on_triton_backend() -> (
    ...
    selector = "accuracy/test_llm_api_autodeploy.py::TestLlama3_1_8B::test_auto_dtype[triton_paged-False-1]"
    cases = bazel_cases_from_manifest(
        _synthetic_autodeploy_manifest(
            selector=selector,
            requirements=["cuda", "model_cache", "triton"],
        )
    )

    assert len(cases) == 1
    assert "requires:triton" in cases[0].tags
    assert TRITON_BACKEND_DEP in cases[0].deps
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "cpp/tensorrt_llm/kernels/gemm/kernel.cu",
            "//cpp:cuda_kernels",
        ),
        (
            "cpp/tensorrt_llm/plugins/common/plugin.cpp",
            "//cpp:nvinfer_plugin_tensorrt_llm",
        ),
        (
            "triton_backend/inflight_batcher_llm/src/backend.cc",
            "//triton_backend:triton_tensorrt_llm_backend",
        ),
    ],
)
def test_select_impacted_native_artifact_owner_mapping(path: str, expected_label: str) -> None:
    assert owner_labels_for_path(path) == (expected_label,)


def test_select_impacted_phase6_keeps_thop_unmodeled_and_kernels_modeled() -> None:
    thop_path = "cpp/tensorrt_llm/thop/custom_op.cpp"
    kernels_path = "cpp/tensorrt_llm/kernels/gemm/kernel.cu"
    query_client = _FakeQueryClient(
        fallback_query_results=["//ci/bazel:fallback_test"],
    )

    assert owner_labels_for_path(kernels_path) == ("//cpp:cuda_kernels",)
    assert owner_labels_for_path(thop_path) == ()

    result = select_impacted(
        repo_root=REPO_ROOT,
        base="upstream/main",
        changed_files=[_changed_file(thop_path)],
    ...
    assert f"unmodeled changed path: {thop_path}" in result.fallback_reasons
    assert any("tests(" in expression for expression in query_client.queries)


def test_select_impacted_unmodeled_native_path_uses_broad_fallback() -> None:
    path = "cpp/tensorrt_llm/unmodeled/native_op.cpp"
    query_client = _FakeQueryClient(
        fallback_query_results=["//ci/bazel:fallback_test"],
    )
    ...
    assert result.candidate_targets == ("//ci/bazel:fallback_test",)
    assert result.selected_targets == ("//ci/bazel:fallback_test", "//ci/bazel:smoke_test")
    assert any(path in reason for reason in result.fallback_reasons)
    assert any("tests(" in expression for expression in query_client.queries)
