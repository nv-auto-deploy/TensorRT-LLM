...
from _model_test_utils import generate_dynamic_shapes
...
def _run_job(
    ...
    dynamic_shapes = generate_dynamic_shapes(batch_size * 2, sequence_len * 2)
    run_test(
        model,
        x,
...
def test_all_reduce_cudagraph(
    model_cls: Type[nn.Module], dist_op_expected: str, bias: bool, device_count: int = 2
):
    ...
    port = mpi_dist.broadcast(dist_common.get_free_port())  # use MPI broadcast to pick a free port
    dist_common.initialize_or_skip(rank, world_size, port)

    print(f"Running test with rank {rank} and world size {world_size}")
    dist_common.init_and_run_process(
