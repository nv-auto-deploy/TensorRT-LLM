...
from tensorrt_llm._torch.auto_deploy.transformations.library.rms_norm import match_rms_norm
...
def test_rms_norm(eps, variant):
    def checker(gm):
        if variant == "flashinfer":
            op = torch.ops.auto_deploy.flashinfer_rms_norm
        else:
            op = torch.ops.auto_deploy.triton_rms_norm
    ...
    gm_transformed = run_test(
        ...
        partial(match_rms_norm, backend=variant),
        checker,
