...
class SimpleMoEForExport(nn.Module):
    ...
    def __init__(self, num_experts: int = 8, hidden_dim: int = 16, inter_dim: int = 32):
        super().__init__()
...
def test_moe_export_with_reduced_experts(num_experts, num_moe_experts_for_export, device):
    """Export with fewer experts then expand — result must match full export."""
    mod = SimpleMoEForExport(num_experts=num_experts).to(device)
    mod.eval()
    ...
    assert len(mod.experts) == num_experts, "Source model experts were not restored"
