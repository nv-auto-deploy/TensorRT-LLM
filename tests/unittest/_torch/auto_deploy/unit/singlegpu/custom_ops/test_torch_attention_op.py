...
def numpy_attention_reference(
    q, k, v, k_cache, v_cache, seq_len, input_pos, cache_loc, seq_start,
    scale=None, logit_cap=None, sliding_window_size=None
):
    ...
    if is_generate:
        ...
        for i in range(batch_size):
            ...
            k_seq = k_np[0, seq_start_i:seq_start_i + seq_len_i].reshape(seq_len_i, n_kv_heads, head_dim)
            v_seq = v_np[0, seq_start_i:seq_start_i + seq_len_i].reshape(seq_len_i, n_kv_heads, v_head_dim)
    ...
    for i in range(batch_size):
        ...
        if q_np.ndim == 3 and q_np.shape[0] > 1:
            ...
            q_seq = q_np[i, :seq_len_i].reshape(seq_len_i, n_heads, head_dim)  # [seq_len, n_heads, head_dim]
        else:
            # Flattened case: all sequences are flattened in the second dimension
            q_seq = q_np[0, seq_start_i:seq_start_i + seq_len_i].reshape(seq_len_i, n_heads, head_dim)
...
class TestTorchBackendAttention:
    ...
    def _create_test_data(self, batch_size, seq_len, n_heads, n_kv_heads, d_head, max_seq_len, cache_offset=0):
        ...
        k = torch.randn(batch_size, seq_len, n_kv_heads, d_head, dtype=self.dtype, device=self.device)
        v = torch.randn(batch_size, seq_len, n_kv_heads, d_head, dtype=self.dtype, device=self.device)

        # Create KV cache
        k_cache = torch.randn(batch_size, max_seq_len, n_kv_heads, d_head, dtype=self.dtype, device=self.device)
        v_cache = torch.randn(batch_size, max_seq_len, n_kv_heads, d_head, dtype=self.dtype, device=self.device)

        # Setup metadata
        input_positions = torch.full((batch_size,), cache_offset, device=self.device, dtype=torch.int)
        ...
        if seq_len == 1:
            ...
            seq_start = torch.arange(0, batch_size * seq_len, seq_len, device=self.device, dtype=torch.int32)
        ...
        return {
            ...
            'seq_len': seq_len_tensor, 'input_pos': input_positions, 
            'cache_loc': cache_loc, 'seq_start': seq_start,
            'k_cache': k_cache, 'v_cache': v_cache
        }

    def _run_attention(self, data, scale=None, logit_cap=None, sliding_window_size=None):
        """Run torch backend attention operation."""
        return torch.ops.auto_deploy.torch_backend_attention_mha_with_cache(
            data['q'], data['k'], data['v'], data['seq_len'], data['input_pos'], 
            data['cache_loc'], data['seq_start'], data['k_cache'], data['v_cache'],
            scale, logit_cap, sliding_window_size
        )
    ...
    def test_combined_features_with_reference(self, seq_len, logit_cap, sliding_window_size):
        """Test combined logit capping and sliding window features against numpy reference."""
        batch_size, n_heads, n_kv_heads, d_head, max_seq_len = 2, 8, 4, 16, 64
        cache_offset = 5  # Have some tokens in cache
        
        data = self._create_test_data(batch_size, seq_len, n_heads, n_kv_heads, d_head, max_seq_len, cache_offset)
        
        # Test with combined features
        output = self._run_attention(data, None, logit_cap, sliding_window_size)
        
        # Compare with numpy reference
        reference = numpy_attention_reference(
            data['q'], data['k'], data['v'], data['k_cache'], data['v_cache'],
            data['seq_len'], data['input_pos'], data['cache_loc'], data['seq_start'],
            None, logit_cap, sliding_window_size
        )
        ...
        assert torch.allclose(output, reference_torch, atol=self.atol, rtol=self.rtol), \
            f"Output doesn't match numpy reference for seq_len={seq_len}, logit_cap={logit_cap}, sliding_window_size={sliding_window_size}"

    def test_feature_differences(self):
        """Test that different feature combinations produce different outputs."""
        batch_size, seq_len, n_heads, n_kv_heads, d_head, max_seq_len = 1, 1, 4, 2, 16, 64
        cache_offset = 8
        
        # Create test data with extreme values to ensure features have visible effects
        data = self._create_test_data(batch_size, seq_len, n_heads, n_kv_heads, d_head, max_seq_len, cache_offset)
        # Amplify values to make feature effects more pronounced
        data['q'] *= 10.0
        data['k'] *= 10.0
        data['k_cache'] *= 5.0
        
        # Test different configurations
        output_baseline = self._run_attention(data)
        output_logit_cap = self._run_attention(data, None, 3.0, None)
        output_sliding_window = self._run_attention(data, None, None, 4)
        output_combined = self._run_attention(data, None, 3.0, 4)
        
        # Verify that different features produce different outputs
        assert not torch.allclose(output_baseline, output_logit_cap, atol=1e-5, rtol=1e-5), \
            "Logit capping should change output"
        
        assert not torch.allclose(output_baseline, output_sliding_window, atol=1e-5, rtol=1e-5), \
            "Sliding window should change output"
        
        assert not torch.allclose(output_baseline, output_combined, atol=1e-5, rtol=1e-5), \
            "Combined features should change output"
    ...
    def test_context_vs_generate_phases(self):
        ...
        context_data = self._create_test_data(batch_size, 4, n_heads, n_kv_heads, d_head, max_seq_len)
        context_output = self._run_attention(context_data)
        
        context_reference = numpy_attention_reference(
            context_data['q'], context_data['k'], context_data['v'], 
            context_data['k_cache'], context_data['v_cache'],
            context_data['seq_len'], context_data['input_pos'], 
            context_data['cache_loc'], context_data['seq_start']
        )
        context_reference_torch = torch.from_numpy(context_reference).to(context_output.device, context_output.dtype)
        
        assert torch.allclose(context_output, context_reference_torch, atol=self.atol, rtol=self.rtol), \
            "Context phase doesn't match reference"
        
        # Test generate phase (single-token)
        generate_data = self._create_test_data(batch_size, 1, n_heads, n_kv_heads, d_head, max_seq_len, 5)
        generate_output = self._run_attention(generate_data)
        
        generate_reference = numpy_attention_reference(
            generate_data['q'], generate_data['k'], generate_data['v'],
            generate_data['k_cache'], generate_data['v_cache'],
            generate_data['seq_len'], generate_data['input_pos'],
            generate_data['cache_loc'], generate_data['seq_start']
        )
        generate_reference_torch = torch.from_numpy(generate_reference).to(generate_output.device, generate_output.dtype)
        
        assert torch.allclose(generate_output, generate_reference_torch, atol=self.atol, rtol=self.rtol), \
