...
- GVA (Grouped Value Attention): q/k have fewer heads than v.
...
def _random_inputs(device, dtype, batch, seq, num_k_heads, num_v_heads, key_dim, value_dim):
    ...
    q/k have num_k_heads, v/a/b/A_log/dt_bias have num_v_heads (GVA when num_v_heads > num_k_heads).
    """
...
def test_decode_only(gdr_env, num_k_heads, num_v_heads):
    ...
    # Reference: preprocess and call fused_recurrent directly
    q_norm, k_norm, g, beta = _preprocess_for_reference(q, k, a, b, A_log, dt_bias)
    q_flat = q_norm.view(batch, num_k_heads, -1)
    k_flat = k_norm.view(batch, num_k_heads, -1)
...
def test_prefill_only(gdr_env, num_k_heads, num_v_heads):
    ...
    _, _, g, beta = _preprocess_for_reference(q, k, a, b, A_log, dt_bias)
