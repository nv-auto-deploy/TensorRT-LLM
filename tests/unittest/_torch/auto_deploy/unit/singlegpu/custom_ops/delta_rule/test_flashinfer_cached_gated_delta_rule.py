"""Unit tests for the flashinfer_cached_gated_delta_rule custom op.

Uses the CuTe DSL GDN decode kernel (requires SM90+ / Hopper GPU).

Covers:
- Decode-only path: batch of single tokens, verify output and final state
  match the pure-torch ``_torch_gated_delta_step`` reference.
...
from tensorrt_llm._torch.auto_deploy.custom_ops.delta_rule.torch_backend_gated_delta import (
...
def test_decode_only(gdr_env):
    ...
    Verifies output and cache state match the pure-torch _torch_gated_delta_step
    reference called directly with gathered initial states.
    """
    device = gdr_env["device"]
    dtype = gdr_env["dtype"]
    # Wider tolerances: CuTe kernel computes softplus/sigmoid on GPU in Float32,
    # may differ slightly from PyTorch's CPU-traced compute_g_beta path.
    atol = 5e-2
    rtol = 5e-2

    batch = 3
    seq = 1
    # CuTe kernel requires K >= 128, V >= 128, V % 32 == 0
    num_heads = 2
    key_dim = 128
    value_dim = 128
    max_batch_size = 6
    ...
    # Run cached op
    y = torch.ops.auto_deploy.flashinfer_cached_gated_delta_rule(
        q,
        k,
        v,
        a,
    ...
    # Reference: pure-torch step for each decode token
    y_ref_list = []
    ...
    for i in range(batch):
        o_ref, s_ref = _torch_gated_delta_step(
            ...
            gathered_before[i].unsqueeze(0).float(),  # [1, H, K, V]
            scale,
        )
    ...
    torch.testing.assert_close(
        after.float(),
        final_state_ref.to(after.dtype).float(),
        atol=atol,
        rtol=rtol,
    )
...
def test_prefill_only(gdr_env):
    ...
    The FlashInfer backend uses the FLA Triton chunked kernel for prefill,
    so this should match chunk_gated_delta_rule exactly.
    """
    device = gdr_env["device"]
    dtype = gdr_env["dtype"]
    atol = 5e-3
    ...
    # Use smaller dims for prefill since it goes through the Triton kernel (no K/V constraints)
    num_heads = 2
    key_dim = 128
    value_dim = 128
    max_batch_size = 4
...
def test_prefill_with_initial_state(gdr_env):
    ...
    y_with_init = torch.ops.auto_deploy.flashinfer_cached_gated_delta_rule(
        q,
        k,
        v,
        a,
    ...
    y_without_init = torch.ops.auto_deploy.flashinfer_cached_gated_delta_rule(
        q,
        k,
        v,
        a,
