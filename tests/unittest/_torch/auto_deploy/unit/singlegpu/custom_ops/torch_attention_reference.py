...
import torch
from tensorrt_llm._torch.auto_deploy.custom_ops.torch_backend_attention import torch_backend_mha_with_cache
...
class TorchAttentionReference:
    ...
    def basic_mha_with_cache(q, k, v, k_cache, v_cache, input_positions, scale=None):
        ...
        seq_start = torch.arange(0, batch_size * seq_len, seq_len, device=q.device, dtype=torch.int32)
        ...
        output_flat = torch_backend_mha_with_cache(
            q_flat, k_flat, v_flat, seq_len_tensor, input_positions, cache_loc, seq_start,
    ...
    @staticmethod
    def flattened_mha_with_cache(q, k, v, seq_len, input_positions, cache_loc, seq_start, k_cache, v_cache, scale=None):
        ...
        return torch_backend_mha_with_cache(
            q, k, v, seq_len, input_positions, cache_loc, seq_start,
            k_cache, v_cache, scale
        )
    ...
    def decode_with_prefilled_cache(q, k_ref, v_ref, k_cache, v_cache, prefill_lengths):
        ...
        # Call torch backend
        output_flat = torch_backend_mha_with_cache(
            q_flat, k_flat, v_flat, seq_len, input_positions, cache_loc, seq_start,
    ...
    @staticmethod
    def mha_with_features(q, k, v, seq_len, input_positions, cache_loc, seq_start, k_cache, v_cache, 
        ...
        return torch_backend_mha_with_cache(
            q, k, v, seq_len, input_positions, cache_loc, seq_start,
            k_cache, v_cache, scale, logit_cap, sliding_window_size
        )
    ...
    def prepare_flattened_inputs(q_list, k_list, v_list, input_positions_list):
        ...
        seq_start = torch.tensor([sum(seq_lengths[:i]) for i in range(len(seq_lengths))], device=device, dtype=torch.int32)
