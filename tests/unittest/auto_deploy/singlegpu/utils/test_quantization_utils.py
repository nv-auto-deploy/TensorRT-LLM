...
def test_fake_fp8_act_quant_export_keeps_prefix_inferred_for_sharding() -> None:
    class Wrapper(torch.nn.Module):
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            return fake_fp8_act_quant(x, block_size=64)
    ...
    for node in gm.graph.nodes:
        if node.target != torch.ops.aten.reshape.default:
            continue
        shape = node.args[1]
        if isinstance(shape, (list, tuple)) and list(shape[-2:]) == [2, 64]:
            grouping_shapes.append(shape)

    assert grouping_shapes
    assert all(list(shape) == [-1, 2, 64] for shape in grouping_shapes)
