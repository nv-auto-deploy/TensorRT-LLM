...
"""Tests for Llama 3 custom model implementation.

This module tests the custom Llama 3 model implementation which uses
auto_deploy custom ops (torch_attention, torch_rope_with_explicit_cos_sin)
for export compatibility. Llama 3 uses GQA with SwiGLU MLP and RMSNorm.
"""
...
from tensorrt_llm._torch.auto_deploy.models.custom.modeling_llama3 import (
    Llama3Attention,
    Llama3DecoderLayer,
    Llama3ForCausalLM,
    Llama3MLP,
...
def _create_small_config() -> LlamaConfig:
...
def _get_hf_decoder_layer_class():
    """Get the HF LlamaDecoderLayer class."""
    try:
        from transformers.models.llama.modeling_llama import (
            LlamaDecoderLayer as HFLlamaDecoderLayer,
        )

        return HFLlamaDecoderLayer
    except ImportError:
        return None


def _get_hf_rotary_class():
    """Get the HF LlamaRotaryEmbedding class."""
    try:
        from transformers.models.llama.modeling_llama import (
            LlamaRotaryEmbedding as HFLlamaRotaryEmbedding,
        )

        return HFLlamaRotaryEmbedding
    except ImportError:
        return None
...
@torch.no_grad()
def test_llama3_rmsnorm_equivalence(B, S, dtype):
    ...
    try:
        ...
        pytest.skip("transformers doesn't have LlamaRMSNorm")

    device = "cuda"
    config = _create_small_config()

    hf_norm = HFLlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    hf_norm.to(device=device, dtype=dtype)
    hf_norm.eval()

    custom_norm = Llama3RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
@pytest.mark.parametrize("dtype", [torch.bfloat16])
@torch.no_grad()
def test_llama3_attention_equivalence(B, S, dtype):
    ...
    if HFAttention is None or HFRotary is None:
        pytest.skip("transformers doesn't have LlamaAttention or LlamaRotaryEmbedding")
    ...
    custom_attn = Llama3Attention(config, layer_idx=0)
...
@pytest.mark.parametrize("dtype", [torch.bfloat16])
@torch.no_grad()
def test_llama3_decoder_layer_equivalence(B, S, dtype):
    ...
    if HFDecoderLayer is None or HFRotary is None:
        pytest.skip("transformers doesn't have LlamaDecoderLayer or LlamaRotaryEmbedding")
    ...
    custom_layer = Llama3DecoderLayer(config, layer_idx=0)
...
@torch.no_grad()
def test_llama3_full_model_equivalence(B, S, dtype, device):
    """Test full model produces numerically equivalent output to HF implementation."""
    HFModel = _get_hf_model_class()
    if HFModel is None:
        pytest.skip("transformers doesn't have LlamaForCausalLM")
    ...
    custom_model = Llama3ForCausalLM(config)
...
@torch.no_grad()
def test_llama3_model_can_be_exported():
...
def test_llama3_gqa_structure():
    ...
    # Llama does NOT have per-head Q/K norms (unlike Qwen3)
    assert not hasattr(attn, "q_norm"), "Llama attention should NOT have q_norm"
...
def test_llama3_state_dict_keys():
    ...
    # Verify NO q_norm/k_norm keys exist (Llama doesn't have these)
    for key in state_dict:
        assert "q_norm" not in key, f"Unexpected q_norm key: {key}"
        assert "k_norm" not in key, f"Unexpected k_norm key: {key}"
