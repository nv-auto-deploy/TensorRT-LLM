...
"""Tests for Qwen3 custom model implementation.

This module tests the custom Qwen3 model implementation which uses
auto_deploy custom ops (torch_attention, torch_rope_with_explicit_cos_sin)
for export compatibility. Qwen3 uses GQA with per-head Q/K normalization.
"""
...
from tensorrt_llm._torch.auto_deploy.models.custom.modeling_qwen3 import (
    Qwen3Attention,
    Qwen3DecoderLayer,
    Qwen3ForCausalLM,
    Qwen3MLP,
...
def _create_small_config() -> Qwen3Config:
...
def _get_hf_model_class():
    """Get the HF Qwen3ForCausalLM class."""
    try:
        from transformers.models.qwen3.modeling_qwen3 import Qwen3ForCausalLM as HFQwen3ForCausalLM

        return HFQwen3ForCausalLM
    except ImportError:
        return None


def _get_hf_attention_class():
    """Get the HF Qwen3Attention class."""
    try:
        from transformers.models.qwen3.modeling_qwen3 import Qwen3Attention as HFQwen3Attention

        return HFQwen3Attention
    except ImportError:
        return None


def _get_hf_mlp_class():
    """Get the HF Qwen3MLP class."""
    try:
...
def _get_hf_decoder_layer_class():
    """Get the HF Qwen3DecoderLayer class."""
    try:
        from transformers.models.qwen3.modeling_qwen3 import (
            Qwen3DecoderLayer as HFQwen3DecoderLayer,
        )

        return HFQwen3DecoderLayer
    except ImportError:
        return None


def _get_hf_rotary_class():
    """Get the HF Qwen3RotaryEmbedding class."""
    try:
        from transformers.models.qwen3.modeling_qwen3 import (
            Qwen3RotaryEmbedding as HFQwen3RotaryEmbedding,
        )

        return HFQwen3RotaryEmbedding
    except ImportError:
        return None
...
@pytest.mark.parametrize("dtype", [torch.bfloat16])
@torch.no_grad()
def test_qwen3_attention_equivalence(B, S, dtype):
    ...
    if HFAttention is None or HFRotary is None:
        pytest.skip("transformers doesn't have Qwen3Attention or Qwen3RotaryEmbedding")
    ...
    custom_attn = Qwen3Attention(config, layer_idx=0)
...
@pytest.mark.parametrize("dtype", [torch.bfloat16])
@torch.no_grad()
def test_qwen3_decoder_layer_equivalence(B, S, dtype):
    ...
    if HFDecoderLayer is None or HFRotary is None:
        pytest.skip("transformers doesn't have Qwen3DecoderLayer or Qwen3RotaryEmbedding")
    ...
    custom_layer = Qwen3DecoderLayer(config, layer_idx=0)
...
@torch.no_grad()
def test_qwen3_full_model_equivalence(B, S, dtype, device):
    """Test full model produces numerically equivalent output to HF implementation."""
    HFModel = _get_hf_model_class()
    if HFModel is None:
        pytest.skip("transformers doesn't have Qwen3ForCausalLM")
...
@torch.no_grad()
def test_qwen3_model_can_be_exported():
...
def test_qwen3_config_registration():
