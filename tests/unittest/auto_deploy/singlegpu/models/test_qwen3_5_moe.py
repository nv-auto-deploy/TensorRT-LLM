...
def _write_mtp_text_config(tmp_path, **overrides):
    config_dir = tmp_path / "mtp_text_config"
    config_dir.mkdir()
    config = _make_small_config(mtp_num_hidden_layers=1, **overrides)
    with (config_dir / "config.json").open("w", encoding="utf-8") as config_file:
        json.dump(config.to_dict(), config_file)
    return config_dir


def _build_mtp_one_model_factory(config_dir) -> EagleOneModelFactory:
...
def _dynamic_shapes_for_mtp_draft_export():
    return {
        "inputs_embeds": {0: Dim.DYNAMIC, 1: Dim.DYNAMIC},
        "position_ids": {0: Dim.DYNAMIC, 1: Dim.DYNAMIC},
        "hidden_states": {0: Dim.DYNAMIC, 1: Dim.DYNAMIC},
        "kwargs": {},
    }
...
def test_mtp_drafter_forward_outputs_are_finite():
    torch.manual_seed(2)
    config = _make_small_config(mtp_num_hidden_layers=1)
    eagle_config = EagleConfig.from_base_config(config, config.model_type)
    drafter = EagleDrafterForCausalLM._from_config(eagle_config, use_cache=False)
    _init_mtp_module_weights(drafter)
...
def test_mtp_drafter_factory_load_or_random_init_contract(tmp_path):
    config_dir = _write_mtp_text_config(tmp_path)
    ...
    assert isinstance(_get_mtp_layer(drafter), Qwen3_5MoeEagleLayer)
...
def test_mtp_one_model_factory_builds_expected_wrapper(tmp_path):
    config_dir = _write_mtp_text_config(tmp_path)
    factory = _build_mtp_one_model_factory(config_dir)
...
def test_mtp_one_model_factory_export_infos_use_eagle_io_contract(tmp_path):
    config_dir = _write_mtp_text_config(tmp_path)
    factory = _build_mtp_one_model_factory(config_dir)
    wrapper = factory.build_model("meta")
