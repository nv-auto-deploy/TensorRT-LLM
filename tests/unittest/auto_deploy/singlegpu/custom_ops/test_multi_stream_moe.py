...
Architecture patterns tested:
  ...
  **Nemotron Ultra V3 (trailing all-reduce)** — Shared expert ending in a
    TP all-reduce collective directly feeding the merge node.  This topology
    is produced by ``fuse_rmsnorm_quant_nvfp4`` on Nemotron Ultra V3 at TP≥2.
    The transform must keep the collective on the main stream; placing it on
    the aux stream causes concurrent cross-rank collectives under monolithic
    CUDA-graph replay (nvbugs/6248757).
