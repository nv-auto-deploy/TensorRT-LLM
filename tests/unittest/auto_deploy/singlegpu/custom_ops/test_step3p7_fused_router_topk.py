...
ops of Step's sigmoid + per-expert-bias top-k router into a single Triton
launch. These tests validate that it is (1) numerically faithful to the
...
@pytest.mark.parametrize("num_experts,top_k", [(288, 8), (256, 8), (8, 2)])
def test_fused_router_topk_matches_reference(num_tokens, num_experts, top_k):
    device = "cuda"
    ...
    router_bias = torch.randn(num_experts, dtype=torch.float32, device=device) * 0.5
    scaling = 3.0
