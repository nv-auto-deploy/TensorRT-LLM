...
def test_mtp_autodeploy_converts_to_eagle3_one_model():
    from tensorrt_llm._torch.auto_deploy.llm_args import LlmArgs

    # Llama doesn't have MTP, but it does not matter for this unit test.
    model = "meta-llama/Meta-Llama-3.1-8B-Instruct"
    args = LlmArgs(
        model=model,
    ...
    assert isinstance(args.speculative_config, Eagle3DecodingConfig)
    assert args.speculative_config.max_draft_len == 3
    assert args.speculative_config.speculative_model == model
    assert args.speculative_config.eagle3_one_model is True
    assert args.speculative_config.eagle3_layers_to_capture == {-1}
    assert args.model_factory == "eagle_one_model"


def test_detect_hidden_states_capture_last_layer_for_llama_eagle3():
    from tensorrt_llm._torch.auto_deploy.llm_args import LlmArgs
