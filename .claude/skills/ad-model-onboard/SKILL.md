...
**Device placement**: models and input tensors that feed a forward must be on GPU (`.cuda()` / `device="cuda"`). Do NOT parametrize `device` over `["cpu", "cuda"]` — use `["cuda"]` only. Exception: tensors that production code keeps on CPU (e.g. `batch_info_host` and similar host metadata read via `.numpy()`/`.item()`) stay on CPU.
...
- Use only `torch_*` prefixed reference ops in AutoDeploy — never `triton_*`, `flashinfer_*`, or `trtllm_*`. Tests run models on GPU (see Phase 6).
