...
class PromptConfig(BaseModel):
    ...
    def validate_sp_kwargs(cls, sp_kwargs):
        """Insert desired defaults for sampling params."""
        return {**cls.model_fields["sp_kwargs"].default_factory(), **sp_kwargs}
...
class ExperimentConfig(BaseSettings):
    ...
    def sync_model_with_args(cls, model_value, info):
        args: _AutoDeployLlmArgs = info.data["args"]
...
def build_llm_from_config(config: ExperimentConfig) -> LLM:
    ...
    # prefetch the checkpoint
    config.args.create_factory().prefetch_checkpoint()
