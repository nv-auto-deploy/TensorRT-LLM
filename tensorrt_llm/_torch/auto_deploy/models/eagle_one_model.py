...
"""Eagle One Model Factory for building combined target+draft models as a single unit.

This factory composes a target model factory and a draft model factory to build
a combined Eagle model that can be used for speculative decoding with a single
...
class EagleOneModelFactory(ModelFactory):
    ...
    def build_from_target(
        ...
        Args:
            ...
            device: Device for resource manager buffers ("meta" for tracing, "cuda" for runtime).
        ...
        # Create draft factory using AutoModelForCausalLMFactory
        # Eagle3 models use "Eagle3LlamaForCausalLM" model_type
        draft_factory = AutoModelForCausalLMFactory(
    ...
    def _load_checkpoint(self, model: nn.Module, device: DeviceLikeType):
        ...
        self.target_factory._load_checkpoint(model.target_model, device)

        # Load draft model weights
        self.draft_factory._load_checkpoint(model.draft_model, device)
