...
def get_model_from_config_patched(config, **kwargs):
    ...
    if re.search(r"Phi-4-mini-instruct", config._name_or_path):
        ...
        return model
    if re.search(r"Phi-4-mini-reasoning", config._name_or_path):
        for _, module in model.named_modules():
            name = type(module).__name__
            if name == "Phi3RotaryEmbedding":
    ...
    if not re.search(pattern, config._name_or_path):
        return model
    for _, module in model.named_modules():
        name = type(module).__name__
        if name == "Phi3RotaryEmbedding":
