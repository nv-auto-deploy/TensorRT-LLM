...
class Eagle3Model(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Input feature fusion: hidden_size -> hidden_size for Eagle3.
        # Technically we could have an fc layer even if we have just one capture layer,
        # but it is not the case for MTPEagle.
...
class Eagle3DrafterForCausalLM(PreTrainedModel):
    ...
    def apply_d2t(self, draft_output_ids: torch.Tensor) -> torch.Tensor:
        if self.model.d2t is not None:
            draft_output_ids = self.model.d2t.data[draft_output_ids] + draft_output_ids
        return draft_output_ids

    def load_weights_from_target(self, target_model):
        if self.load_embedding_from_target:
            assert self.model.embed_tokens is None
            self.model.embed_tokens = target_model.model.embed_tokens
        if self.load_lm_head_from_target:
            assert self.lm_head is None
...
class EagleWrapperOutput(ModelOutput):
    ...
    # Not used for sampling. Used for compatibility.
    ...
    # new_tokens_lens: [batch_size]. Number of accepted tokens per request.
    # Equivalent to num_newly_accepted_tokens but named for MTPSampler compatibility.
    ...
    # num_accepted_tokens: [batch_size]. The total number of accepted tokens in each batch after this iteration.
    # (Kept for backward compatibility with prefill-only mode)
    num_accepted_tokens: Optional[torch.Tensor] = None

    # num_newly_accepted_tokens: [batch_size]. The number of newly accepted tokens in each batch from this iteration.
    # (Kept for backward compatibility with prefill-only mode)
    num_newly_accepted_tokens: Optional[torch.Tensor] = None
...
class EagleWrapper(nn.Module):
    ...
    def sample_and_verify(
        ...
        if input_ids.device.type == "meta":
            # During capture, return simple dummy values with correct shapes
            # This avoids data-dependent operations that can't run on meta tensors
            output_ids = self.sample_greedy(target_logits)
            num_newly_accepted_tokens = seq_len - num_previously_accepted
            num_accepted_tokens = torch.full(
                (batch_size,), seq_len, dtype=torch.long, device=input_ids.device
            )
            # last_logits_3d: use the last position's logit as a placeholder
            last_logits_3d = target_logits[:, -1:, :]
        ...
        for i in range(batch_size):
            # 1. Previously accepted tokens (skip the first one)
    ...
    def _forward_prefill_only(self, input_ids, position_ids, **kwargs):
        ...
        # The valid prefix within each tensor has length: num_accepted_tokens[i] + max_draft_len
        ...
        return EagleWrapperOutput(
            ...
            num_newly_accepted_tokens=num_newly_accepted_tokens,
        )
