...
class EagleWrapper(nn.Module):
    ...
    def _debug_print_kwargs(
        ...
        indent: str = "  ",
        max_elements: int = 20,
        force_print_keys: set[str] | None = None,
    ) -> None:
        ...
        Args:
            ...
            force_print_keys: Set of keys to always print full values for, bypassing max_elements
        """
        if force_print_keys is None:
            force_print_keys = set()

        print(f"\n{title}")
        for key, val in kwargs.items():
            if isinstance(val, torch.Tensor):
                # Bypass max_elements limit for keys in force_print_keys
                force_print = key in force_print_keys
                tensor_max = val.numel() + 1 if force_print else max_elements
                if old_kwargs is not None and key in old_kwargs:
                    old_val = old_kwargs[key]
                    # Print old->new comparison if both tensors are small enough (or forced)
                    small_enough = old_val.numel() <= max_elements and val.numel() <= max_elements
                    if isinstance(old_val, torch.Tensor) and (force_print or small_enough):
                        print(f"{indent}{key}: {old_val.tolist()} -> {val.tolist()}")
                    else:
                        self._debug_print_tensor(key, val, indent, tensor_max)
                else:
                    self._debug_print_tensor(key, val, indent, tensor_max)
            else:
                if old_kwargs is not None and key in old_kwargs:
    ...
    def _debug_print_draft_inputs(
        ...
        old_kwargs: dict | None = None,
        output: Any = None,
        force_print_cache_loc: bool = False,
    ) -> None:
        ...
        Args:
            ...
            force_print_cache_loc: If True, always print full cache_loc values (bypass max_elements)
        """
        print("\n" + "=" * 60)
        ...
        if kwargs is not None:
            force_print_keys = {"cache_loc"} if force_print_cache_loc else None
            self._debug_print_kwargs(
                "  kwargs:", kwargs, old_kwargs, indent="    ", force_print_keys=force_print_keys
            )
    ...
    def _count_context_sequences(
        ...
        print(
            f"[EagleWrapper._count_context_sequences] num_sequences={num_sequences}, "
            f"num_context={num_context}, num_spec_dec={num_sequences - num_context}"
        )
    ...
    def _store_target_hidden_states(
        ...
        """
        # Verify resource_manager is the correct type
        ...
        print(f"[EagleWrapper._store_target_hidden_states] num_tokens: {num_tokens}")
    ...
    def _forward_with_kv_cache(self, input_ids, position_ids, **kwargs):
        ...
        draft_kwargs = self._filter_kwargs_for_submodule(kwargs, "draft_model")
