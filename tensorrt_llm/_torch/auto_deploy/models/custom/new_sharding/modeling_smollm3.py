...
"""SmolLM3 model with explicit sharding hint ops.

This is a rewrite of the original modeling_smollm3.py where all shardable
...
SmolLM3 is Llama-like with GQA, SwiGLU MLP, RMSNorm, and per-layer NoPE
(no RoPE) toggling via ``config.no_rope_layers``.
...
class SmolLM3MLP(nn.Module):
    """MLP layer for SmolLM3 (SwiGLU) with sharding hints.
...
class SmolLM3Attention(nn.Module):
    """Grouped Query Attention for SmolLM3 with sharding hints and per-layer RoPE toggling.
