...
"""DeepSeekV2 model with explicit sharding hint ops.

WARNING: tested only on up to 8 layers (8xH100 machine can't fit the full
DeepSeek-V2.5 model, the output is not fully verified). The sharding pipeline
processes all nodes correctly (59 nodes at 8 layers) but coherent end-to-end
output has not been validated due to memory constraints.

Based on the original modeling_deepseek_v2.py. All shardable operations use
...
class DeepSeekV2MLP(nn.Module):
    """MLP layer for DeepSeekV2 (SwiGLU activation) with sharding hints.
...
class DeepSeekV2MoEGate(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.topk_method == "greedy":
            ...
            group_scores = scores.view(n_tokens, self.n_group, -1).max(dim=-1).values
...
class DeepSeekV2MoE(nn.Module):
    """Mixture of Experts layer for DeepSeekV2 with sharding hints.
...
class DeepSeekV2Attention(nn.Module):
    """Multi-head Latent Attention (MLA) for DeepSeekV2 with sharding hints.

    MLA sharding strategy:
      q_proj (q_lora_rank=None) -> tp_mode="colwise"
      q_a_proj                  -> tp_mode="none" (replicated latent projection)
      q_a_layernorm             -> unchanged
      q_b_proj                  -> tp_mode="colwise" (shard by num_heads)
      kv_a_proj_with_mqa        -> tp_mode="none" (replicated latent projection)
    ...
    def forward(
        ...
        # Q projection: latent projections are replicated, q_b_proj/q_proj is colwise
        if self.q_lora_rank is None:
        ...
        # Apply RoPE (weights pre-permuted to NeoX format at load time)
