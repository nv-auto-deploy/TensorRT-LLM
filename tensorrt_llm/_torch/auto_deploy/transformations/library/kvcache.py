...
def resize_kv_cache(
    ...
    try:
        ...
        ad_logger.info(f"Free memory before forward pass (MB): {free_mem_pre // 1024**2}")
        egm(*cm.args)
        free_mem_post, _ = torch.cuda.mem_get_info()
        ad_logger.info(f"Free memory after forward pass (MB): {free_mem_post // 1024**2}")

        memory_for_forward_pass = free_mem_pre - free_mem_post
        ad_logger.info(f"Memory for forward pass (MB): {memory_for_forward_pass // 1024**2}")
