...
class ADEngine(ModelEngine):
    ...
    def _prepare_inputs(
        ...
        for request in gen_requests:
            ...
            # For one-model spec dec, verify enough pages are allocated for internal draft iterations
            if self.spec_config is not None and self.spec_config.spec_dec_mode.use_one_engine():
                max_draft_len = self.spec_config.max_draft_len
                # Worst case: current seq_len + (max_draft_len - 1) internal draft iterations
                max_pages_needed = (
                    seq_len_with_cache[-1] + max_draft_len - 1 + page_size - 1
                ) // page_size
                assert len(cache_indices) >= max_pages_needed, (
                    f"Insufficient pages for one-model spec dec: have {len(cache_indices)} pages, "
                    f"need {max_pages_needed} (seq_len={seq_len_with_cache[-1]}, max_draft_len={max_draft_len})"
                )
