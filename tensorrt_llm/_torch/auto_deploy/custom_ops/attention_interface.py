...
class SequenceInfo:
    ...
    def _active_host_update_args(
        ...
        ``active_args_override`` lets a caller narrow the global active-argument set when it knows
        the next consumer only reads a subset of graph inputs. The caller is responsible for passing
        an override that includes every host argument the next consumer may read.
        """
        needs_d2h_sync = [
            k + self._host_suffix
            for k in arg_names
        ...
        inactive_args = active_args_override - set(self._active_args)
        if inactive_args:
            raise ValueError(
                "active_args_override must be a subset of active graph arguments "
                f"(got inactive arguments: {sorted(inactive_args)})."
            )
