# NOTE: Temporary copy from flashinfer PR #2370
# (https://github.com/flashinfer-ai/flashinfer/pull/2370)
# Remove this file once flashinfer bundles the GDN decode kernel upstream.
...
# ============================================================================
# BF16 State Kernels (direct cooperative loads, no cp.async)
# ============================================================================
# These kernels handle bfloat16 state pools by using thread-cooperative element
# loads with bf16->fp32 conversion, replacing cp.async which requires matching
# source/destination element widths. Internal compute remains fp32.


@cute.kernel
def gdn_decode_kernel_small_batch_bf16state(
    h0_source: cute.Tensor,
    smem_layout_staged: cute.Layout,
    num_v_tiles: cutlass.Constexpr[int],
    q: cute.Tensor,
    ...
    """Small batch bf16 state kernel: direct cooperative loads, bf16 store."""
    tidx, _, _ = cute.arch.thread_idx()
    in_warp_tid = tidx % 32
    warp_idx = cute.arch.warp_idx()
    warp_idx = cute.arch.make_warp_uniform(warp_idx)
    ...
    if pool_idx >= 0:
        ...
        for v_tile_offset in range(num_v_tiles_per_block):
            ...
            # Direct cooperative load: bf16 -> fp32
            for load_iter in range(NUM_K_ITERS_SMALL):
                flat_idx = tidx + load_iter * 128
                k_idx_load = flat_idx // TILE_V_SMALL_NT
                v_idx_load = flat_idx % TILE_V_SMALL_NT
                if k_idx_load < TILE_K_NT:
                    v_global_load = v_tile * TILE_V_SMALL_NT + v_idx_load
                    sData[(k_idx_load, v_idx_load, stage)] = cutlass.Float32(
                        h0_source[(pool_idx, i_hv, k_idx_load, v_global_load)]
                    )
            cute.arch.barrier()
            ...
            for k_iter in range(NUM_K_ITERS_SMALL):
                ...
                if k_write < TILE_K_NT:
                    h_val = sData[(k_write, v_write, stage)]
                    v_global_write = v_tile * TILE_V_SMALL_NT + v_write
                    h0_source[(pool_idx, i_hv, k_write, v_global_write)] = cutlass.BFloat16(h_val)

            cute.arch.barrier()


@cute.kernel
def gdn_decode_kernel_big_batch_bf16state(
    h0_source: cute.Tensor,
    smem_layout_staged: cute.Layout,
    num_v_tiles: cutlass.Constexpr[int],
    q: cute.Tensor,
    ...
    """Large batch bf16 state kernel: direct cooperative loads, bf16 store."""
    tidx, _, _ = cute.arch.thread_idx()
    in_warp_tid = tidx % 32
    warp_idx = cute.arch.warp_idx()
    warp_idx = cute.arch.make_warp_uniform(warp_idx)
    ...
    if pool_idx >= 0:
        ...
        for v_tile in range(num_v_tiles):
            ...
            # Direct cooperative load: bf16 -> fp32
            for load_iter in range(NUM_K_ITERS_NT):
                flat_idx = tidx + load_iter * 256
                k_idx_load = flat_idx // TILE_V_NT
                v_idx_load = flat_idx % TILE_V_NT
                if k_idx_load < TILE_K_NT:
                    v_global_load = v_tile * TILE_V_NT + v_idx_load
                    sData[(k_idx_load, v_idx_load, stage)] = cutlass.Float32(
                        h0_source[(pool_idx, i_hv, k_idx_load, v_global_load)]
                    )
            cute.arch.barrier()
...
@cute.jit
def run_gdn_decode_kernel_small_batch_bf16state(
    cu_seqlens: cute.Tensor,
    q: cute.Tensor,
    k: cute.Tensor,
    v: cute.Tensor,
    ...
    gdn_decode_kernel_small_batch_bf16state(
        h0_source,
        smem_layout_small,
        num_v_tiles_small,
        q,
...
@cute.jit
def run_gdn_decode_kernel_big_batch_bf16state(
    cu_seqlens: cute.Tensor,
    q: cute.Tensor,
    k: cute.Tensor,
    v: cute.Tensor,
    ...
    gdn_decode_kernel_big_batch_bf16state(
        h0_source,
        base_smem_layout,
        num_v_tiles,
        q,
...
# ============================================================================
# Pool-Indexed Decode API (zero-copy access to state cache)
# ============================================================================


@functools.cache
def _get_compiled_decode_kernel_nontranspose_pooled(
    D: int,
    pool_size: int,
    T: int,
    H: int,
    ...
    """Cache compiled kernel for pool-indexed decode configuration."""
    return {}


@flashinfer_api
def gated_delta_rule_decode_pooled(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    r"""Pool-indexed Gated Delta Rule Decode kernel (zero-copy state access).

    Unlike :func:`gated_delta_rule_decode`, this function takes the full state
    pool and slot indices directly, avoiding gather/scatter copies. The kernel
    reads and writes state in-place at the indexed pool positions.

    Args:
        q (torch.Tensor):
            Current query of shape ``[D, 1, H, K]``. Must be float16/bfloat16.
        k (torch.Tensor):
            Current key of shape ``[D, 1, H, K]``. Must be float16/bfloat16.
        v (torch.Tensor):
            Current value of shape ``[D, 1, HV, V]``. Must be float16/bfloat16.
        state_pool (torch.Tensor):
            Full state cache of shape ``[pool_size, HV, K, V]`` (k-major layout).
            Can be float32 or bfloat16. Updated in-place at indexed positions.
        state_indices (torch.Tensor):
            Slot indices of shape ``[D]`` mapping each decode sequence to its
            position in the state pool. Must be int32.
        A_log (torch.Tensor):
            Log decay parameter of shape ``[HV]``. Must be float32.
        a (torch.Tensor):
            Input-dependent decay of shape ``[D, 1, HV]``. Must be float16/bfloat16.
        dt_bias (torch.Tensor):
            Decay bias of shape ``[HV]``. Must be bfloat16 or float32.
        b (torch.Tensor):
            Update gate (beta) input of shape ``[D, 1, HV]``. Must be float16/bfloat16.
        scale (Optional[float]):
            Scale factor for queries. If None, defaults to ``1 / sqrt(K)``.
        use_qk_l2norm (bool):
            Whether to apply L2 normalization to q and k. Default: ``False``.

    Returns:
        torch.Tensor: Output tensor of shape ``[D, 1, HV, V]``.

    Note:
        - Requires SM90 (Hopper) architecture
        - State is updated in-place at indexed positions (no gather/scatter)
        - Supports float32 and bfloat16 state dtypes
    """
    D, T, H, K = q.shape
    assert T == 1, f"Decode only supports T=1, got T={T}"
    _, _, HV, V = v.shape
    ...
    # Validate shapes
    assert state_pool.ndim == 4 and state_pool.shape[1:] == (HV, K, V), (
        f"Expected state_pool shape [pool_size, HV={HV}, K={K}, V={V}], got {state_pool.shape}"
    )
    assert state_indices.shape == (D,), (
        f"Expected state_indices shape [{D}], got {state_indices.shape}"
    )
    ...
    # Validate dtypes
    assert q.dtype in (torch.float16, torch.bfloat16), f"q must be float16/bfloat16, got {q.dtype}"
    assert state_pool.dtype in (torch.float32, torch.bfloat16), (
        f"state_pool must be float32 or bfloat16, got {state_pool.dtype}"
    )
    assert A_log.dtype == torch.float32, f"A_log must be float32, got {A_log.dtype}"

    is_bf16_state = state_pool.dtype == torch.bfloat16

    if scale is None:
        scale = K**-0.5

    # Allocate output (kernel outputs bfloat16)
    output = torch.zeros((D, T, HV, V), dtype=torch.bfloat16, device=q.device)

    # Ensure state_indices is int32
    if state_indices.dtype != torch.int32:
        state_indices = state_indices.to(torch.int32)

    # Dummy cu_seqlens (unused but required by launcher signature)
    cu_seqlens = torch.zeros(D + 1, dtype=torch.int32, device=q.device)

    # Compile kernel with TVM FFI (cached)
    cache_key = (D, pool_size, T, H, HV, K, V, q.dtype, state_pool.dtype, scale, use_qk_l2norm)
    cache = _get_compiled_decode_kernel_nontranspose_pooled(*cache_key)

    if "compiled" not in cache:
        stream = cuda.CUstream(torch.cuda.current_stream().cuda_stream)

        # Choose kernel based on decode batch size and state dtype
        use_small_batch = D < SMALL_BATCH_THRESHOLD_NT

        if is_bf16_state:
            if use_small_batch:
                run_func = run_gdn_decode_kernel_small_batch_bf16state
            else:
                run_func = run_gdn_decode_kernel_big_batch_bf16state
        else:
            if use_small_batch:
                run_func = run_gdn_decode_kernel_small_batch_nontranspose
            else:
        ...
        # Convert tensors to CuTe format for compilation only
        h0_source_tensor = from_dlpack(state_pool, assumed_align=16)
        A_log_tensor = from_dlpack(A_log, assumed_align=16)
        a_tensor = from_dlpack(a, assumed_align=16)
        dt_bias_tensor = from_dlpack(dt_bias, assumed_align=16)
        q_tensor = from_dlpack(q, assumed_align=16)
        ...
        b_tensor = from_dlpack(b, assumed_align=16)
        o_tensor = from_dlpack(output, assumed_align=16)
        h0_indices_tensor = from_dlpack(state_indices, assumed_align=16)
        cu_seqlens_tensor = from_dlpack(cu_seqlens, assumed_align=16)
    ...
    # Run kernel directly with PyTorch tensors
    stream = cuda.CUstream(torch.cuda.current_stream().cuda_stream)
    compiled(
        cu_seqlens,
        q,
    ...
    # No state copy needed -- kernel writes in-place via pool_idx

    # Convert output to q's dtype if needed
        ...
        for v_tiles in range(num_v_tiles):
            ...
            if not disable_state_update:
                for row in range(0, TILE_V_MTP, 4):
                    row_offset = tidx // 32
                    for i in range(vec_size):
                        gDst_h0[(0, row + row_offset, i * 32 + lane_id, v_tiles)] = sData[
                            (row + row_offset, i * 32 + lane_id, stage)
                        ]
