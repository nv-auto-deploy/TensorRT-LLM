...
"""Benchmark RMSNorm custom op across FlashInfer, Triton, and Torch backends.

Usage::

    python -m tensorrt_llm._torch.auto_deploy.custom_ops.benchmarks.bench_rms_norm
    python -m tensorrt_llm._torch.auto_deploy.custom_ops.benchmarks.bench_rms_norm --dtype half
"""
...
# ---------------------------------------------------------------------------
# Provider dtype support for RMSNorm
# ---------------------------------------------------------------------------
...
def benchmark_rms_norm(
    ...
    if provider == "flashinfer":
        fn = lambda: torch.ops.auto_deploy.flashinfer_rms_norm(x, w, eps)  # noqa: E731
    elif provider == "triton":
        fn = lambda: torch.ops.auto_deploy.triton_rms_norm(x, w, eps)  # noqa: E731
    elif provider == "torch":
        fn = lambda: torch.ops.auto_deploy.torch_rmsnorm(x, w, eps)  # noqa: E731
    else:
...
def main():
    parser = make_arg_parser(description="Benchmark RMSNorm custom op across backends.")
    ...
    report = perf_report(
        Benchmark(
            ...
            ylabel="ms",
            plot_name=f"rms-norm-performance-{args.dtype}",
            args={},
        )
    )

    report(
        lambda num_tokens, hidden_size, provider: benchmark_rms_norm(
