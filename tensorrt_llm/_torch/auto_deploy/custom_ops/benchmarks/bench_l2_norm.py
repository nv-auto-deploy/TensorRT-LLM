...
"""Benchmark L2Norm custom op across Torch and FLA backends.

Usage::

    python -m tensorrt_llm._torch.auto_deploy.custom_ops.benchmarks.bench_l2_norm
    python -m tensorrt_llm._torch.auto_deploy.custom_ops.benchmarks.bench_l2_norm --dtype half
"""
...
def benchmark_l2_norm(
    ...
    if provider == "torch":
        fn = lambda: torch.ops.auto_deploy.torch_l2norm(x, eps)  # noqa: E731
    elif provider == "fla":
        fn = lambda: torch.ops.auto_deploy.fla_l2norm(x, eps)  # noqa: E731
    else:
...
def main():
    parser = make_arg_parser(description="Benchmark L2Norm custom op across backends.")
    ...
    report = perf_report(
        Benchmark(
            ...
            ylabel="ms",
            plot_name=f"l2-norm-performance-{args.dtype}",
            args={},
        )
    )

    report(
        lambda num_tokens, hidden_size, provider: benchmark_l2_norm(
