...
def _write_paged_cache_rows(
    ...
    if _is_flash_mla_model1_cache(cache):
        ...
        page_ids = []
        page_offsets = []
        for logical_pos_tensor in logical_positions.detach().cpu():
        ...
        _write_flash_mla_model1_rows_to_cache(
            cache,
            values,
            torch.tensor(page_ids, dtype=torch.long, device=values.device),
            torch.tensor(page_offsets, dtype=torch.long, device=values.device),
        )
        return
