...
_FLASH_MLA_PREFILL_HEAD_DIM = 512
_FLASH_MLA_PREFILL_TOPK_MULTIPLE = 128
_FLASH_MLA_DECODE_TOPK_MULTIPLE = 128
_FLASH_MLA_MODEL1_HEAD_DIM = 512
_FLASH_MLA_MODEL1_NOPE_DIM = 448
_FLASH_MLA_MODEL1_ROPE_DIM = 64
_FLASH_MLA_MODEL1_QUANT_TILE = 64
_FLASH_MLA_MODEL1_NUM_TILES = 7
_FLASH_MLA_MODEL1_TOKEN_DATA_BYTES = _FLASH_MLA_MODEL1_NOPE_DIM + 2 * _FLASH_MLA_MODEL1_ROPE_DIM
_FLASH_MLA_MODEL1_SCALE_BYTES = 8
_FLASH_MLA_MODEL1_BYTES_PER_TOKEN = (
    _FLASH_MLA_MODEL1_TOKEN_DATA_BYTES + _FLASH_MLA_MODEL1_SCALE_BYTES
)
_FLASH_MLA_FP8_MAX = 448.0
...
def _is_flash_mla_model1_cache(cache: torch.Tensor) -> bool:
    return (
        cache.dim() == 3
        and cache.dtype == torch.uint8
        and cache.shape[-1] == _FLASH_MLA_MODEL1_BYTES_PER_TOKEN
    )


def _validate_flash_mla_model1_cache_inputs(
    q: torch.Tensor,
    kv: torch.Tensor,
    name: str,
    cache: torch.Tensor,
) -> None:
    _validate_rank(name, cache, 3)
    if cache.device != q.device:
        raise ValueError(f"{name} must be on {q.device}, got {cache.device}")
    if kv.shape[-1] != _FLASH_MLA_MODEL1_HEAD_DIM:
        raise ValueError(
            "FlashMLA DeepSeek V4 sparse backend requires kv head dimension "
            f"{_FLASH_MLA_MODEL1_HEAD_DIM}, got {kv.shape[-1]}"
        )
    if not _is_flash_mla_model1_cache(cache):
        raise TypeError(
            f"{name} must be a uint8 FlashMLA MODEL1 cache with token shape "
            f"[{_FLASH_MLA_MODEL1_BYTES_PER_TOKEN}], got dtype={cache.dtype} "
            f"and shape={tuple(cache.shape)}"
        )


def _pack_flash_mla_model1_rows(
    rows: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    if rows.dim() != 2 or rows.shape[-1] != _FLASH_MLA_MODEL1_HEAD_DIM:
        raise ValueError(
            "FlashMLA MODEL1 packing requires rows with shape [num_rows, "
            f"{_FLASH_MLA_MODEL1_HEAD_DIM}], got {tuple(rows.shape)}"
        )

    rows_bf16 = rows.to(torch.bfloat16).contiguous()
    nope = rows_bf16[:, :_FLASH_MLA_MODEL1_NOPE_DIM].float()
    nope_tiles = nope.view(-1, _FLASH_MLA_MODEL1_NUM_TILES, _FLASH_MLA_MODEL1_QUANT_TILE)
    absmax = nope_tiles.abs().amax(dim=-1)
    exponent = torch.ceil(torch.log2(torch.clamp_min(absmax / _FLASH_MLA_FP8_MAX, 1.0e-4)))
    scales = torch.pow(torch.full_like(exponent, 2.0), exponent)
    quantized = torch.clamp(
        nope_tiles / scales.unsqueeze(-1), -_FLASH_MLA_FP8_MAX, _FLASH_MLA_FP8_MAX
    )
    nope_bytes = quantized.to(torch.float8_e4m3fn).view(torch.uint8)
    nope_bytes = nope_bytes.reshape(rows.shape[0], _FLASH_MLA_MODEL1_NOPE_DIM).contiguous()
    ...
    scale_bytes = torch.clamp(exponent + 127.0, 0.0, 255.0).to(torch.uint8).contiguous()
    return nope_bytes, rope_bytes, scale_bytes


def _write_flash_mla_model1_rows_to_cache(
    cache: torch.Tensor,
    values: torch.Tensor,
    ...
    if values.numel() == 0:
        return
    if not _is_flash_mla_model1_cache(cache):
        raise TypeError("cache must be a FlashMLA MODEL1 uint8 cache")

    flat_values = values.reshape(-1, values.shape[-1])
    flat_page_ids = page_ids.reshape(-1).to(torch.long)
    flat_page_offsets = page_offsets.reshape(-1).to(torch.long)
    if valid is None:
        flat_valid = None
    else:
        flat_valid = valid.reshape(-1).to(torch.bool)

    nope_bytes, rope_bytes, scale_bytes = _pack_flash_mla_model1_rows(flat_values)
    zero_scale = torch.zeros(
        scale_bytes.shape[0],
        1,
        dtype=torch.uint8,
        device=scale_bytes.device,
    )
    scale_bytes = torch.cat((scale_bytes, zero_scale), dim=-1)
    flat_cache = cache.view(-1)
    block_size = int(cache.shape[1])
    block_offsets = flat_page_ids * int(cache.stride(0))
    token_data_start = block_offsets + flat_page_offsets * _FLASH_MLA_MODEL1_TOKEN_DATA_BYTES
    scale_start = block_offsets + block_size * _FLASH_MLA_MODEL1_TOKEN_DATA_BYTES
    scale_start = scale_start + flat_page_offsets * _FLASH_MLA_MODEL1_SCALE_BYTES

    def scatter_bytes(start_offsets: torch.Tensor, packed: torch.Tensor) -> None:
        byte_offsets = torch.arange(packed.shape[-1], dtype=torch.long, device=packed.device)
        offsets = start_offsets.unsqueeze(-1) + byte_offsets
        flat_offsets = offsets.reshape(-1)
        flat_packed = packed.reshape(-1)
        if flat_valid is not None:
            valid_bytes = flat_valid.unsqueeze(-1).expand_as(packed).reshape(-1)
            existing = flat_cache.gather(0, flat_offsets)
            flat_packed = torch.where(valid_bytes, flat_packed, existing)
        flat_cache.scatter_(0, flat_offsets, flat_packed)

    scatter_bytes(token_data_start, nope_bytes)
    scatter_bytes(token_data_start + _FLASH_MLA_MODEL1_NOPE_DIM, rope_bytes)
    scatter_bytes(scale_start, scale_bytes)
...
def _host_page_ids_and_offsets(
    cache: torch.Tensor,
    seq_idx: int,
    logical_positions_host: torch.Tensor,
    cu_num_pages_host: torch.Tensor,
    cache_loc_host: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor]:
    logical_positions_host = logical_positions_host.to(torch.long).flatten()
    if logical_positions_host.numel() == 0:
        return logical_positions_host, logical_positions_host

    negative_mask = logical_positions_host < 0
    if negative_mask.any():
        first_bad = int(logical_positions_host[negative_mask.nonzero()[0].item()].item())
        raise ValueError(f"logical_pos must be non-negative, got {first_bad}")

    tokens_per_block = int(cache.shape[1])
    page_ordinals = torch.div(logical_positions_host, tokens_per_block, rounding_mode="floor")
    page_offsets = logical_positions_host.remainder(tokens_per_block)
    page_start = int(cu_num_pages_host[seq_idx].item())
    page_end = int(cu_num_pages_host[seq_idx + 1].item())
    page_table_idxs = page_start + page_ordinals
    invalid_mask = page_table_idxs >= page_end
    if invalid_mask.any():
        first_bad_idx = int(invalid_mask.nonzero()[0].item())
        logical_pos = int(logical_positions_host[first_bad_idx].item())
        page_ordinal = int(page_ordinals[first_bad_idx].item())
        raise ValueError(
            f"Sequence {seq_idx} logical position {logical_pos} needs page ordinal "
            f"{page_ordinal}, but only {page_end - page_start} page(s) are active"
        )

    page_ids = cache_loc_host.index_select(0, page_table_idxs.to(torch.long)).to(torch.long)
...
def _write_paged_cache_rows(
    ...
    if values.numel() == 0:
        return
    logical_positions_host = torch.arange(
        input_pos,
        input_pos + values.shape[0],
        dtype=torch.long,
    )
    page_ids, page_offsets = _host_page_ids_and_offsets(
        cache,
        seq_idx,
        logical_positions_host,
        cu_num_pages_host,
        cache_loc_host,
    )
    if _is_flash_mla_model1_cache(cache):
        _write_flash_mla_model1_rows_to_cache(
            cache,
            values,
            page_ids.to(device=values.device),
            page_offsets.to(device=values.device),
        )
        return

    cache[
        page_ids.to(device=cache.device),
        page_offsets.to(device=cache.device),
...
def _flatten_decode_tokens(
    ...
    if tensor.shape[0] > seq_start and tensor.shape[0] != 1:
        return tensor[seq_start : seq_start + num_decode, :1].reshape(num_decode, *tensor.shape[2:])
    return tensor.reshape(-1, *tensor.shape[2:])[token_start : token_start + num_decode]


def _flatten_decode_position_ids(
    position_ids: torch.Tensor,
    num_decode: int,
    token_start: int = 0,
    seq_start: int = 0,
) -> torch.Tensor:
    if position_ids.shape[0] > seq_start and position_ids.shape[0] != 1:
        return position_ids[seq_start : seq_start + num_decode, :1].reshape(-1).to(torch.long)
    return position_ids.reshape(-1)[token_start : token_start + num_decode].to(torch.long)
...
def _write_decode_cache_rows(
    ...
    if values.numel() == 0:
        return
    page_ids, page_offsets, page_valid = _decode_page_ids_and_offsets(
        cache, seq_idx, input_pos, cu_num_pages, cache_loc
    )
    write_valid = (input_pos.to(torch.long) >= 0) & page_valid
    if valid is not None:
        write_valid = write_valid & valid.reshape(write_valid.shape).to(torch.bool)
    if _is_flash_mla_model1_cache(cache):
        _write_flash_mla_model1_rows_to_cache(cache, values, page_ids, page_offsets, write_valid)
        return
    rows = values.to(cache.dtype)
    if valid is not None:
        previous_rows = cache[page_ids, page_offsets]
        row_valid = write_valid
        while row_valid.dim() < rows.dim():
            row_valid = row_valid.unsqueeze(-1)
        rows = torch.where(row_valid, rows, previous_rows)
    cache[page_ids, page_offsets] = rows
...
def _update_decode_compressed_caches(
    ...
    if _is_flash_mla_model1_cache(mhc_cache):
        _write_decode_cache_rows(
            mhc_cache,
            compressed_rows,
            seq_idx,
...
def _flash_mla_deepseek_v4_sparse_attention_decode_with_cache(
    q: torch.Tensor,
    kv: torch.Tensor,
    attn_sink: torch.Tensor,
    topk_idxs: torch.Tensor,
    ...
    q_flat = q.reshape(-1, *q.shape[2:])
    q_decode = _flatten_decode_tokens(q, num_decode, token_start, seq_start)
    kv_decode = _flatten_decode_tokens(kv, num_decode, token_start, seq_start)
    topk_decode = _flatten_decode_tokens(topk_idxs, num_decode, token_start, seq_start)
    seq_idx_decode = torch.arange(
        seq_start, seq_start + num_decode, dtype=torch.long, device=input_pos.device
    )
    input_pos_decode = input_pos.reshape(-1)[seq_start : seq_start + num_decode].to(torch.long)
    position_ids_decode = _flatten_decode_position_ids(
        position_ids, num_decode, token_start, seq_start
    )
    ...
    if compress_ratio:
        ...
        assert max_compressed_len is not None
        assert rope_dim is not None
        compressor_kv_decode = _flatten_decode_tokens(
            compressor_kv, num_decode, token_start, seq_start
        )
        compressor_gate_decode = _flatten_decode_tokens(
            compressor_gate, num_decode, token_start, seq_start
        )
        _update_decode_compressed_caches(
            compressor_kv_decode,
            compressor_gate_decode,
        ...
        mode = _compression_mode(compress_ratio)
        if mode.uses_indexer:
            indexer_compressor_kv_decode = _flatten_decode_tokens(
                indexer_compressor_kv,
                num_decode,
                token_start,
                seq_start,
        ...
        indexer_q_decode = _flatten_decode_tokens(indexer_q, num_decode, token_start, seq_start)
        indexer_weights_decode = _flatten_decode_tokens(
            indexer_weights, num_decode, token_start, seq_start
        )
        extra_indices, extra_topk_length = _build_flash_mla_compressed_decode_indices(
            topk_decode,
            indexer_q_decode,
            indexer_weights_decode,
            indexer_compressor_kv_cache,
    ...
    if window_size is not None:
        indices, topk_length = _build_flash_mla_local_decode_indices(
            swa_cache,
            seq_idx_decode,
            input_pos_decode,
            cu_num_pages,
        ...
        indices, topk_length = _build_flash_mla_topk_decode_indices(
            swa_cache,
            seq_idx_decode,
            topk_decode,
            cu_num_pages,
    ...
    decode_output = _flash_mla_sparse_decode(
        q_decode,
        swa_cache,
        attn_sink,
        indices,
        topk_length,
        softmax_scale,
        extra_k_cache=mhc_cache if extra_indices is not None else None,
        extra_indices=extra_indices,
        extra_topk_length=extra_topk_length,
    )

    output_flat = torch.zeros_like(q_flat)
    output_flat[token_start : token_start + num_decode].copy_(decode_output)
    output = output_flat.view_as(q)
    if out is not None:
        out.copy_(output)
        return out.new_empty(0)
...
def _validate_flash_mla_sparse_prefill_contract(op: Callable) -> None:
    supports_dsv4_contract = getattr(op, "_supports_dsv4_contract", None)
    try:
        parameters = inspect.signature(op).parameters
    except (TypeError, ValueError) as exc:
        raise RuntimeError(
            "FlashMLA sparse prefill must expose the DeepSeek V4 contract. "
            "Rebuild the pinned vLLM FlashMLA dependency."
        ) from exc

    required_kwargs = {"attn_sink", "topk_length", "out"}
    missing_kwargs = required_kwargs.difference(parameters)
    if supports_dsv4_contract is False or missing_kwargs:
        missing_str = ", ".join(sorted(missing_kwargs)) or "_supports_dsv4_contract"
        raise RuntimeError(
            "FlashMLA sparse prefill is available, but it does not expose the "
            f"DeepSeek V4 contract ({missing_str} missing). Rebuild the pinned "
            "vLLM FlashMLA dependency."
        )


@lru_cache(maxsize=1)
def _get_flash_mla_sparse_prefill() -> Optional[Callable]:
    try:
        from tensorrt_llm.flash_mla import flash_mla_sparse_fwd
    except (ImportError, ModuleNotFoundError, OSError, RuntimeError):
        return None
    _validate_flash_mla_sparse_prefill_contract(flash_mla_sparse_fwd)
    return flash_mla_sparse_fwd


def _flash_mla_sparse_prefill_head_count(
    num_heads: int,
    capability: tuple[int, int],
) -> Optional[int]:
    if capability[0] >= 10:
        return 128 if num_heads <= 128 else None
    if capability[0] == 9:
        return ((num_heads + 63) // 64) * 64
    return None


def _prepare_flash_mla_sparse_prefill_inputs(
    q: torch.Tensor,
    kv: torch.Tensor,
    topk_idxs: torch.Tensor,
    ...
    batch_size, seq_len, num_heads, head_dim = q.shape
    kv_rows = kv.shape[1]
    k_select = topk_idxs.shape[-1]

    q_flat = q.reshape(batch_size * seq_len, num_heads, head_dim).contiguous()
    kv_flat = kv.reshape(batch_size * kv_rows, 1, head_dim).contiguous()

    batch_offsets = torch.arange(batch_size, dtype=torch.long, device=q.device)
    batch_offsets = (batch_offsets * kv_rows).view(batch_size, 1, 1)
    topk_long = topk_idxs.to(torch.long)
    valid_topk = (topk_long >= 0) & (topk_long < kv_rows)
    indices = torch.where(topk_long >= 0, topk_long + batch_offsets, topk_long)
    indices = torch.where(valid_topk, indices, torch.full_like(indices, -1))
    indices = indices.reshape(batch_size * seq_len, 1, k_select).to(torch.int32).contiguous()

    padded_topk = (
        (k_select + _FLASH_MLA_PREFILL_TOPK_MULTIPLE - 1) // _FLASH_MLA_PREFILL_TOPK_MULTIPLE
    ) * _FLASH_MLA_PREFILL_TOPK_MULTIPLE
    if padded_topk != k_select:
        pad = torch.full(
            (indices.shape[0], 1, padded_topk - k_select),
            -1,
            dtype=torch.int32,
            device=q.device,
        )
    ...
    return q_flat, kv_flat, indices, topk_length


def _flash_mla_sparse_prefill(
    q: torch.Tensor,
    kv: torch.Tensor,
    attn_sink: torch.Tensor,
    topk_idxs: torch.Tensor,
    ...
    flash_mla_sparse_fwd = _get_flash_mla_sparse_prefill()
    if flash_mla_sparse_fwd is None:
        raise RuntimeError(
            "FlashMLA DeepSeek V4 sparse backend requires the FlashMLA sparse prefill kernel."
        )
    if not q.is_cuda or not kv.is_cuda:
        raise RuntimeError("FlashMLA DeepSeek V4 sparse backend requires CUDA q and kv tensors.")
    if q.dtype != torch.bfloat16 or kv.dtype != torch.bfloat16:
        raise RuntimeError(
            "FlashMLA DeepSeek V4 sparse backend requires bfloat16 q and kv tensors."
        )
    if q.shape[-1] != _FLASH_MLA_PREFILL_HEAD_DIM:
        raise RuntimeError(
            "FlashMLA DeepSeek V4 sparse backend requires head_dim="
            f"{_FLASH_MLA_PREFILL_HEAD_DIM}, got {q.shape[-1]}."
        )
    if topk_idxs.shape[-1] <= 0:
        raise RuntimeError("FlashMLA DeepSeek V4 sparse backend requires at least one top-k row.")
    if not torch.cuda.is_available():
        raise RuntimeError("FlashMLA DeepSeek V4 sparse backend requires CUDA availability.")

    capability = torch.cuda.get_device_capability(q.device)
    padded_heads = _flash_mla_sparse_prefill_head_count(q.shape[2], capability)
    if padded_heads is None:
        raise RuntimeError(
            "FlashMLA DeepSeek V4 sparse backend supports Hopper and Blackwell head layouts; "
            f"got compute capability sm_{capability[0]}{capability[1]} and {q.shape[2]} heads."
        )

    q_flat, kv_flat, indices, topk_length = _prepare_flash_mla_sparse_prefill_inputs(
        q, kv, topk_idxs
    )
    num_heads = q_flat.shape[1]
    if padded_heads != num_heads:
        q_padded = q_flat.new_zeros(q_flat.shape[0], padded_heads, q_flat.shape[2])
        q_padded[:, :num_heads].copy_(q_flat)
        q_flat = q_padded
        ...
        attn_sink_padded[:num_heads].copy_(attn_sink.to(torch.float32))
        attn_sink_for_kernel = attn_sink_padded
    else:
        attn_sink_for_kernel = attn_sink.to(torch.float32).contiguous()
    ...
    output, _, _ = flash_mla_sparse_fwd(
        ...
        indices,
        softmax_scale,
        d_v=_FLASH_MLA_PREFILL_HEAD_DIM,
        attn_sink=attn_sink_for_kernel,
...
@lru_cache(maxsize=1)
def _get_flash_mla_decode() -> Optional[tuple[Callable, Callable]]:
    try:
        from tensorrt_llm.flash_mla import flash_mla_with_kvcache, get_mla_metadata
    except (ImportError, ModuleNotFoundError, OSError, RuntimeError):
        return None
    return flash_mla_with_kvcache, get_mla_metadata


def _flash_mla_sparse_decode_head_count(
    num_heads: int,
    capability: tuple[int, int],
) -> Optional[int]:
    if capability[0] < 9:
...
def _decode_positions_to_flash_mla_indices(
    cache: torch.Tensor,
    seq_idx: torch.Tensor,
    positions: torch.Tensor,
    cu_num_pages: torch.Tensor,
    ...
    positions_valid = positions.to(torch.long) >= 0
    if valid is not None:
        positions_valid = positions_valid & valid
    positions_valid = positions_valid & page_valid
    block_size = int(cache.shape[1])
    indices = page_ids * block_size + page_offsets
    indices = torch.where(positions_valid, indices, torch.full_like(indices, -1))
    return indices.to(torch.int32).contiguous(), positions_valid


def _pad_flash_mla_decode_indices(indices: torch.Tensor) -> torch.Tensor:
    width = int(indices.shape[-1])
    padded_width = (
        (width + _FLASH_MLA_DECODE_TOPK_MULTIPLE - 1) // _FLASH_MLA_DECODE_TOPK_MULTIPLE
    ) * _FLASH_MLA_DECODE_TOPK_MULTIPLE
    ...
    return torch.cat((indices, pad), dim=-1).contiguous()


def _build_flash_mla_local_decode_indices(
    cache: torch.Tensor,
    seq_idx: torch.Tensor,
    input_pos: torch.Tensor,
    cu_num_pages: torch.Tensor,
    ...
    offsets = torch.arange(window_size, dtype=torch.long, device=input_pos.device)
    input_pos_long = input_pos.to(torch.long)
    lengths = torch.minimum(input_pos_long + 1, torch.full_like(input_pos_long, window_size))
    lengths = lengths.clamp(min=0)
    start_positions = input_pos_long - lengths + 1
    positions = start_positions.unsqueeze(1) + offsets.view(1, -1)
    valid = offsets.view(1, -1) < lengths.unsqueeze(1)
    indices, valid = _decode_positions_to_flash_mla_indices(
        cache,
        seq_idx,
        positions,
        cu_num_pages,
    ...
    topk_length = valid.sum(dim=-1).to(torch.int32).contiguous()
    return _pad_flash_mla_decode_indices(indices).unsqueeze(1), topk_length
...
def _build_flash_mla_compressed_decode_indices(
    topk_decode: torch.Tensor,
    indexer_q_decode: torch.Tensor,
    indexer_weights_decode: torch.Tensor,
    indexer_compressor_kv_cache: torch.Tensor,
    ...
    if mode.uses_indexer:
        ...
        selected_rows = torch.arange(
            max_compressed_len,
            dtype=torch.long,
            device=topk_decode.device,
        )
        selected_rows = selected_rows.view(1, -1).expand(topk_decode.shape[0], -1)
        compressed_len = ((input_pos + 1) // compress_ratio).clamp(max=max_compressed_len)
        compressed_valid = selected_rows < compressed_len.unsqueeze(1)
        compressed_positions = selected_rows * compress_ratio
...
def _flash_mla_sparse_decode(
    ...
    flash_mla_decode = _get_flash_mla_decode()
    if flash_mla_decode is None:
        raise RuntimeError(
            "FlashMLA DeepSeek V4 sparse backend requires the FlashMLA decode kernel."
        )
    flash_mla_with_kvcache, get_mla_metadata = flash_mla_decode
    if q_decode.dtype != torch.bfloat16:
        raise RuntimeError(
            f"FlashMLA DeepSeek V4 sparse decode requires bfloat16 q tensors, got {q_decode.dtype}."
        )
    if q_decode.shape[-1] != _FLASH_MLA_MODEL1_HEAD_DIM:
        raise RuntimeError(
            "FlashMLA DeepSeek V4 sparse decode requires head_dim="
            f"{_FLASH_MLA_MODEL1_HEAD_DIM}, got {q_decode.shape[-1]}."
        )
    if not _is_flash_mla_model1_cache(swa_cache):
        raise RuntimeError("FlashMLA sparse decode requires a packed uint8 SWA cache.")
    if extra_k_cache is not None and not _is_flash_mla_model1_cache(extra_k_cache):
        raise RuntimeError("FlashMLA sparse decode requires a packed uint8 extra cache.")
    if indices.shape[-1] <= 0:
        raise RuntimeError("FlashMLA sparse decode requires at least one primary index.")
    if not q_decode.is_cuda or not swa_cache.is_cuda:
        raise RuntimeError("FlashMLA DeepSeek V4 sparse decode requires CUDA tensors.")
    if not torch.cuda.is_available():
        raise RuntimeError("FlashMLA DeepSeek V4 sparse decode requires CUDA availability.")

    capability = torch.cuda.get_device_capability(q_decode.device)
    padded_heads = _flash_mla_sparse_decode_head_count(q_decode.shape[1], capability)
    if padded_heads is None:
        raise RuntimeError(
            "FlashMLA DeepSeek V4 sparse decode supports Hopper and Blackwell with "
            f"64 or 128 query heads; got sm_{capability[0]}{capability[1]} and "
            f"{q_decode.shape[1]} heads."
        )

    num_heads = q_decode.shape[1]
    if padded_heads != num_heads:
        q_padded = q_decode.new_zeros(q_decode.shape[0], padded_heads, q_decode.shape[2])
        q_padded[:, :num_heads].copy_(q_decode)
        q_decode = q_padded
        attn_sink_padded = torch.zeros(padded_heads, dtype=torch.float32, device=attn_sink.device)
        attn_sink_padded[:num_heads].copy_(attn_sink.to(torch.float32))
        attn_sink = attn_sink_padded
    else:
        attn_sink = attn_sink.to(torch.float32).contiguous()

    q_decode = q_decode.unsqueeze(1).contiguous()
    ...
    output, _ = flash_mla_with_kvcache(
        ...
        block_table=None,
        cache_seqlens=None,
        head_dim_v=_FLASH_MLA_MODEL1_HEAD_DIM,
        ...
        attn_sink=attn_sink,
        extra_k_cache=extra_k_cache.unsqueeze(-2) if extra_k_cache is not None else None,
        extra_indices_in_kvcache=extra_indices.contiguous() if extra_indices is not None else None,
        extra_topk_length=extra_topk_length.contiguous() if extra_topk_length is not None else None,
    )
    return output[:, 0, :num_heads].contiguous()
...
@torch.library.custom_op(
    "auto_deploy::flashmla_deepseek_v4_sparse_attention_with_cache",
    mutates_args=(
        "swa_cache",
        "mhc_cache",
        "compressor_kv_cache",
...
def flashmla_deepseek_v4_sparse_attention_with_cache(
    ...
    """FlashMLA-backed paged cached DeepSeek V4 sparse attention.

    This backend op is selected by AD graph lowering.  It intentionally does not
    fall back to the torch reference path: unsupported batch shapes should fail
    at the backend boundary so the semantic source op remains backend-neutral.
    """
    del slot_idx, last_page_len
    _validate_deepseek_v4_sparse_attention_inputs(q, kv, attn_sink, topk_idxs)
    _validate_flash_mla_model1_cache_inputs(q, kv, "swa_cache", swa_cache)
    _validate_flash_mla_model1_cache_inputs(q, kv, "mhc_cache", mhc_cache)
    _validate_rank("compressor_kv_cache", compressor_kv_cache, 3)
    _validate_rank("compressor_gate_cache", compressor_gate_cache, 3)
    _validate_rank("indexer_compressor_kv", indexer_compressor_kv, 3)
    _validate_rank("indexer_compressor_gate", indexer_compressor_gate, 3)
    ...
    if num_prefill == 0 and num_decode > 0:
        return _flash_mla_deepseek_v4_sparse_attention_decode_with_cache(
            q,
            kv,
            attn_sink,
            topk_idxs,
    ...
    for seq_idx in range(num_prefill):
        ...
        if input_pos_i != 0:
            raise RuntimeError(
                "FlashMLA DeepSeek V4 sparse backend currently supports fresh prefill "
                f"spans only; sequence {seq_idx} starts at input_pos={input_pos_i}."
            )
        ...
        if compress_ratio:
            ...
            kv_source = torch.cat((kv_seq.unsqueeze(0), compressed_kv), dim=1)
        else:
            kv_source = _prefill_kv_source(kv, kv_seq, seq_idx, num_seq)

        output_flat[flat_start : flat_start + seq_len_i] = _flash_mla_sparse_prefill(
            q_seq.unsqueeze(0),
            kv_source,
            attn_sink,
            topk_seq.unsqueeze(0),
    ...
    if num_decode > 0:
        decode_output = _flash_mla_deepseek_v4_sparse_attention_decode_with_cache(
            q,
            kv,
            attn_sink,
            topk_idxs,
        ...
        output_flat[num_prefill_tokens:active_tokens].copy_(
            decode_output.reshape(-1, *q.shape[2:])[num_prefill_tokens:active_tokens]
        )
...
@flashmla_deepseek_v4_sparse_attention_with_cache.register_fake
def flashmla_deepseek_v4_sparse_attention_with_cache_fake(
    q: torch.Tensor,
    kv: torch.Tensor,
    attn_sink: torch.Tensor,
    topk_idxs: torch.Tensor,
    ...
    return torch_deepseek_v4_sparse_attention_with_cache_fake(
        q,
        kv,
        attn_sink,
        topk_idxs,
...
@AttentionRegistry.register("flashmla_deepseek_v4_sparse")
class FlashMLADeepSeekV4SparseAttention(DeepSeekV4SparseAttention):
    """DeepSeek V4 sparse attention descriptor lowered to the FlashMLA backend op."""

    @classmethod
    def get_cached_attention_op(cls) -> MHACallable:
        return torch.ops.auto_deploy.flashmla_deepseek_v4_sparse_attention_with_cache.default

    @classmethod
    def get_cache_initializers(
        cls, source_attn_node: Node, cache_config: KvCacheConfig
    ) -> ResourceHandlerDict:
        handlers = super().get_cache_initializers(source_attn_node, cache_config)
        kv_node = extract_op_args(source_attn_node, "kv")[0]
        kv_fake: FakeTensor = kv_node.meta["val"]
        head_dim = int(kv_fake.shape[-1])
        if head_dim != _FLASH_MLA_MODEL1_HEAD_DIM:
            raise RuntimeError(
                "FlashMLA DeepSeek V4 sparse backend requires kv head dimension "
                f"{_FLASH_MLA_MODEL1_HEAD_DIM}, got {head_dim}."
            )
        handlers["swa_cache"] = PagedResourceHandler(
            _FLASH_MLA_MODEL1_BYTES_PER_TOKEN,
            dtype=torch.uint8,
        )
        handlers["mhc_cache"] = PagedResourceHandler(
            _FLASH_MLA_MODEL1_BYTES_PER_TOKEN,
            dtype=torch.uint8,
        )
        return handlers
