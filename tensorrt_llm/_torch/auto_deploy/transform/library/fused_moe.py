...
def _stack_nvfp4_trtllm_gen_moe_weights(gm: GraphModule) -> int:
    ...
    for node in matched_nodes:
        ...
        if is_gated_mlp:
            ...
            fc1_stacked = torch.cat([w3_stacked, w1_stacked], dim=1).contiguous()
        else:
            fc1_stacked = w1_stacked

        fc2_stacked = w2_stacked

        # Pad weights if necessary
        fc1_n_dim = fc1_stacked.shape[1]
        fc1_k_dim = fc1_stacked.shape[2] * 2  # Unpack
        fc1_n_padded = _round_up(fc1_n_dim, weight_alignment)
        fc1_k_padded = _round_up(fc1_k_dim, input_hidden_alignment)

        if fc1_n_padded > fc1_n_dim or fc1_k_padded > fc1_k_dim:
            # Pad [E, N, K/2] -> [E, N_padded, K_padded/2]
            fc1_stacked = torch.nn.functional.pad(
                fc1_stacked,
                (0, (fc1_k_padded - fc1_k_dim) // 2, 0, fc1_n_padded - fc1_n_dim),
            )

        fc2_n_dim = fc2_stacked.shape[1]
        fc2_k_dim = fc2_stacked.shape[2] * 2
        fc2_n_padded = _round_up(fc2_n_dim, weight_alignment)
        fc2_k_padded = _round_up(fc2_k_dim, weight_alignment)

        if fc2_n_padded > fc2_n_dim or fc2_k_padded > fc2_k_dim:
            fc2_stacked = torch.nn.functional.pad(
                fc2_stacked,
                (0, (fc2_k_padded - fc2_k_dim) // 2, 0, fc2_n_padded - fc2_n_dim),
            )

        # Apply weight shuffling for TRTLLM-Gen
        fc1_shuffled = _shuffle_weight_for_trtllm_gen(fc1_stacked, is_gated=is_gated_mlp)
        fc2_shuffled = _shuffle_weight_for_trtllm_gen(fc2_stacked, is_gated=False)
        ...
        with graph.inserting_before(node):
            ...
            kwargs = {
                ...
                "top_k": top_k,
                "intermediate_size": fc1_n_padded // 2 if is_gated_mlp else fc1_n_padded,
