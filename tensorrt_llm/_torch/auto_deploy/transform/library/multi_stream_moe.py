...
def _execute_op_in_aux_stream(gm: GraphModule, target_ops) -> Tuple[GraphModule, int]:
    graph = gm.graph
    num_replaced = 0
    ...
    for n in target_nodes:
        ...
        with graph.inserting_after(n):
            new_node = graph.call_function(
                torch.ops.auto_deploy.trtllm_moe_fused_aux, args=n.args, kwargs=n.kwargs
            )
...
class MultiStreamMOE(BaseTransform):
    ...
    def _apply(
        ...
        gm, num_matches = _execute_op_in_aux_stream(gm, [torch.ops.auto_deploy.trtllm_moe_fused])
