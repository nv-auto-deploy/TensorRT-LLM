...
## Generalize Hidden-State Capture Seed Selection

- Commit before task: `04cb31c2dacc79f52ed7ae603565ebb2223070d4`
- Task:
  - Port the cleaner hidden-state capture boundary selection from the
    Mistral4 Eagle branch, using
    `debug/qwen35-mtp/hidden_state_capture_inputs_embeds_handoff.md` as the
    guide.
  - Remove the Qwen-specific assumption that `inputs_embeds` is only valid when
    it is the first placeholder.
- Plan:
  - Update `identify_regions_between_residuals()` to choose the residual-chain
    seed by graph structure:
    - prefer the placeholder feeding an `aten.embedding` op;
    - otherwise use the placeholder named `inputs_embeds`;
    - otherwise return a minimal `[first_placeholder, output]` boundary list.
  - Keep unrelated node-utils changes used for sharding detection intact.
  - Add CPU-only boundary-selection tests and rerun Qwen MTP hidden-state
    capture tests.
- Changes:
  - Replaced the special-case Qwen comment/path in
    `tensorrt_llm/_torch/auto_deploy/utils/node_utils.py` with generic seed
    selection.
  - Added node-utils tests covering:
    - standard `input_ids -> aten.embedding` graphs;
    - an unused `input_ids` placeholder before a live `inputs_embeds`
      placeholder;
    - the defensive fallback when neither seed exists.
- Verification:
  - Sandboxed pytest initially failed before collection due MPI/socket
    initialization; reran focused tests outside the sandbox.
  - `pytest -sv tests/unittest/auto_deploy/singlegpu/utils/test_node_utils_sharding.py -k identify_regions 2>&1 | tee debug/qwen35-mtp/logs/test_identify_regions_inputs_embeds.log`
    - Result: `3 passed, 16 deselected, 3 warnings in 1.60s`.
  - `pytest -sv tests/unittest/auto_deploy/singlegpu/models/test_qwen3_5_moe.py -k 'mtp_target_hidden_state_capture or mtp_target_export_uses_inputs_embeds_as_live_seed' 2>&1 | tee debug/qwen35-mtp/logs/test_qwen3_5_moe_mtp_hidden_state_capture.log`
    - Result: `3 passed, 52 deselected, 3 warnings in 3.84s`.
  - `pytest -sv tests/unittest/auto_deploy/singlegpu/models/test_qwen3_5_moe.py -k mtp 2>&1 | tee debug/qwen35-mtp/logs/test_qwen3_5_moe_mtp_after_hidden_state_seed_cleanup.log`
    - Result: `14 passed, 41 deselected, 3 warnings in 4.22s`.
  - Attempted smoke:
    `pytest -sv tests/unittest/auto_deploy/singlegpu/smoke/test_ad_speculative_decoding.py::test_detect_hidden_states_capture_last_layer_for_mtp_eagle_one_model 2>&1 | tee debug/qwen35-mtp/logs/test_ad_specdec_hidden_state_capture_smoke.log`
    - Blocked before hidden-state capture by missing local fake checkpoint:
      `/lustre/fs1/portfolios/coreai/projects/coreai_comparch_autodeploy/autodeploy_data/llm-models-fake/llama-3.1-model/Llama-3.1-8B-Instruct`.
- Interpretation:
  - Hidden-state detection no longer depends on placeholder ordering.
  - The Qwen MTP target export/capture path still finds the expected residual
    nodes and rewrites the requested capture layer after the cleanup.
  - The generic speculative-decoding smoke remains environment-blocked here,
    not behavior-blocked.
- Current status:
  - Hidden-state capture seed cleanup is implemented and focused unit coverage
    passes.
  - No commit has been created yet; current HEAD remains
    `04cb31c2dacc79f52ed7ae603565ebb2223070d4`.
